...
export const getPendingProjects = async (token: string): Promise<ProjectResponse[]> => {
  const response = await axios.get(`${API_BASE_URL}/projects/pending`, {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });
...
export const getProjectsByTechnicianId = async (technicianId: number, token: string): Promise<ProjectResponse[]> => {
  const response = await axios.get(`${API_BASE_URL}/projects/technician/${technicianId}`, {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });
...
export const acceptProject = async (
  ...
  const response = await axios.post(`${API_BASE_URL}/projects/${projectId}/accept`, request, {
    headers: {
      'Authorization': `Bearer ${token}`
    }
  });
