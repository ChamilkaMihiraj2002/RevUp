...
import { getAppointmentsByCustomerId, updateAppointment, type AppointmentResponse } from "@/services/appointmentService"
...
export default function TechnicianDashboard() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const appointmentsData = await getAppointmentsByCustomerId(userData.userId, accessToken)
            setAppointments(appointmentsData)

            // Fetch vehicle data for each appointment
            const vehicleIds = [...new Set(appointmentsData.map(apt => apt.vehicleId))]
    ...
    const handleStartService = async (appointmentId: number) => {
        ...
        try {
            await updateAppointment(appointmentId, { status: 'ONGOING' }, accessToken)
    ...
    const handleCompleteService = async (appointmentId: number) => {
        ...
        try {
            await updateAppointment(appointmentId, { status: 'COMPLETED' }, accessToken)
            // Refresh data to show updated status
            await fetchData()
    ...
    const scheduledAppointments = appointments.filter(apt => apt.status === 'SCHEDULED' || apt.status === 'ONGOING')
                ...
                <main className="flex-1">
                    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-muted-foreground mt-2">Manage your assigned appointments</p>
                        </div>
                                    ...
                                    <Card>
                                        <CardContent className="py-12">
                                            <div className="text-center">
                                                ...
                                                <p className="text-muted-foreground">You don't have any scheduled or ongoing appointments</p>
                                            </div>
                                        </CardContent>
                                    </Card>
                                ) : (
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div>
                                                            <CardTitle className="flex items-center space-x-3">
                                                                <span>{vehicle ? `${vehicle.model} (${vehicle.year})` : 'Loading...'}</span>
                                                            ...
                                                            <CardDescription>
                                                                ...
                                                                <br />
                                                                Scheduled: {new Date(appointment.scheduledStart).toLocaleString()}
                                                            </CardDescription>
                                                        </div>
                                                    </div>
                                                </CardHeader>
                                                ...
                                                <CardContent className="space-y-6">
                                                    ...
                                                    <div className="flex justify-end pt-4 border-t">
                                                        {appointment.status === 'SCHEDULED' && (
                                                            <Button 
                                                                onClick={() => handleStartService(appointment.appointmentId)}
                                                        ...
                                                        {appointment.status === 'ONGOING' && (
                                                            <Button 
                                                                onClick={() => handleCompleteService(appointment.appointmentId)}
                        ...
                        {activeTab === "completed" && (
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Appointments you've completed today</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    {completedAppointments.length === 0 ? (
                                        <div className="text-center py-12">
                                            ...
                                            <p className="text-muted-foreground">You haven't completed any appointments today</p>
                                        </div>
                                    ) : (
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            <div>
                                                                <p className="font-medium">
                                                                    {vehicle ? `${vehicle.model} (${vehicle.year})` : 'Loading...'}
                                                                </p>
                                                                <p className="text-sm text-muted-foreground">
                                                                    {vehicle && `Registration: ${vehicle.registrationNo}`}
                                                                </p>
                                                                <p className="text-sm text-muted-foreground">
                                                                    Completed: {new Date(appointment.scheduledEnd).toLocaleString()}
                                                                </p>
                                                            </div>
                                                            <Badge variant="outline" className={getStatusColor(appointment.status)}>
                                    ...
                                    {appointments.length === 0 ? (
                                        ...
                                        <div className="space-y-4">
                                            {appointments.map((appointment) => {
                                                const vehicle = vehicles[appointment.vehicleId]
                                                return (
                                                    <div key={appointment.appointmentId} className="flex items-center justify-between p-4 border rounded-lg">
                                                        <div>
                                                            <p className="font-medium">
                                                                {new Date(appointment.scheduledStart).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                                            </p>
                                                            <p className="text-sm text-muted-foreground">
                                                                {vehicle ? `${vehicle.model} (${vehicle.year}) - ${vehicle.registrationNo}` : 'Loading...'}
                                                            </p>
                                                        </div>
                                                        <Badge variant="outline" className={getStatusColor(appointment.status)}>
                                                            {appointment.status}
